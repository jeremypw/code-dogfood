...
public class Scratch.FolderManager.ProjectInfoManager : Object {
    public class ProjectInfo : Object {
        const string PROJECT_INFO_SCHEMA_ID = Constants.PROJECT_NAME + ".Projects";
        public string path {
            owned get {
                return project.path;
            }
        ...
        construct {
            var path_prefix = Path.DIR_SEPARATOR_S + PROJECT_INFO_SCHEMA_ID.replace (".", Path.DIR_SEPARATOR_S) + Path.DIR_SEPARATOR_S;
            var settings_path = path_prefix +
            ...
            project_settings.bind ("expanded", project, "expanded", DEFAULT);
        }

        public void save_doc_info () {
        warning ("Project info save doc info");
